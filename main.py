...
def run_final():
    ...
    # Если воскресенье (6) и время 22:00 (час 22) ИЛИ запущен ручной режим видео
    if (now_msk.weekday() == 6 and now_msk.hour == 22) or "--video" in sys.argv:
        if now_msk.weekday() == 6 and now_msk.hour == 22:
            print("🕒 АВТО-РЕЖИМ: Воскресенье 22:00 МСК.")
        else:
